...
Plugin vẫn xuất bytecode Java 21. Các server Minecraft/Paper/Folia 26.x cần chạy bằng Java 25,
vì vậy ma trận tương thích build và kiểm thử trên cả JDK 21 lẫn JDK 25.
...
1. Cài Java 21 cho server 1.21.x, hoặc Java 25 khi Paper/Folia 26.x yêu cầu.
...
| `/taixiu rollover <high\|low\|cashout>` | Dùng hoặc nhận khoản rollover escrow |
| `/taixiu nhoi <tai\|xiu\|nhantien>` | Alias tiếng Việt của rollover |
...
Permission động `taixiu.maxbet.<amount>` chọn tier cược cao nhất và
`taixiu.tax.discount.<points>` trừ trực tiếp số điểm phần trăm thuế. `taixiu.tax.bypass`
vẫn được ưu tiên. Rollover dùng `taixiu.rollover`; bảo hiểm chuỗi thua dùng
